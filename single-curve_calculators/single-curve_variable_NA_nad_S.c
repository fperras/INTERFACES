...
//The structure is given in a file named "structure.txt" which lists the coordinates of the spins form the support in a 5-column: x y z NA S format
...
//These are the variables that need to be set by the user
    int N_atoms = 1322; //number of surface atoms in the structure.txt file
    int spin = 1; //spin quantum number of the surface atoms, multiplied by a factor of 2
    float RDD_1A = 6434.96709; //dipolar coupling at a distance of 1A. (reduce further by a factor of 1.8 if it is a S-RE(SP)DOR experiment)
...
//variable declarations. Do not change time_steps, max_time, or init_time if you wish to maintain compatibility with INTERFACES
    ...
    float max_time = 0.044, init_time= 0.0002, DSnorm[time_steps+1], DS, RDD,  betaD, alpha, beta, gamma;
    float x[N_atoms], y[N_atoms], z[N_atoms], xy[N_atoms], alphaD[N_atoms], NA[N_atoms], order_parameter[N_atoms];
    FILE *fp, *struct_fp;
    char filename[30], buffer[256];

//Here the program reads the structure file, which is just a list of x y z coordinates, NA, and order parameter in a 5-column text file
//It then converts these into a z distance, xy distance, and alpha angle.
        ...
        for(i=0;i<N_atoms;i++){
            ...
            sscanf(buffer,"%f  %f  %f %f %f\n",&x[i],&y[i],&z[i],&NA[i],&order_parameter[i]);
...
//Here the program creates the RESPDOR data vector
        ...
        for(t=0; t<time_steps; t++){  //start of time loop
            ...
            for(i=0; i<betasteps; i++){  //start of beta loop (cosinusoidal to avoid powder average)
               ...
               for(alpha=0.; alpha<2.*pi; alpha = alpha+ 2.*pi/alphasteps){  //start of alpha loop
                    ...
                    DS=1.;
                    for(j=0;j<N_atoms;j++){
                        RDD=order_parameter[j]*RDD_1A/pow(xy[j]*xy[j]+pow(z[j],2.),1.5);
                        betaD=atan(xy[j]/(z[j]));
